"""
Pipeline for strong passwords: score - chunk - generate honeywords.
...
print("\n[2/3] Scoring with zxcvbn and selecting strong passwords...")
strength = []
for row in tqdm(df.itertuples(), total=len(df), desc="Scoring", unit="pw"):
    strength.append(zxcvbn(row.pw)["score"])
df["strength"] = strength

strong_pw = df.sort_values(by="strength", ascending=False).head(NUM_USER)
strong_pw.to_csv(f"strong_pw_{NUM_USER}_breach.csv", index=False)
print(f"Saved strong_pw_{NUM_USER}_breach.csv")
...
start = time.time()
sweetwords = chaffing_by_qwen4b_chunk(strong_pw_chunks)
elapsed = time.time() - start
print(f"Time taken: {elapsed:.2f} seconds")

res = [([key] + val)[:NUM_SWEETWORDS] for key, val in sweetwords.items()]
out = f"honeywords_Qwen4B_psm_chunk_{NUM_USER}_strong.csv"
