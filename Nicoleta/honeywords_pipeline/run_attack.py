"""
Run the attacker against sweetwords from honeywords_GPT3_10k_breach.csv and
write a JSON output.

CSV format: real_pw,1.,honey1,2.,honey2,
"""
...
CSV_PATH = Path(__file__).parent / "honeywords_GPT3_10k_breach.csv" # path to the honeywords CSV
...
def load_sweetword_lists(csv_path: Path, seed: int = 67) -> list[SweetwordList]:
    ...
    with open(csv_path, newline="", encoding="utf-8") as f:
        ...
        for idx, row in enumerate(reader):
            words = [row[i] for i in range(0, len(row), 2) if row[i].strip()]
            if not words:
                continue
            real_password = words[0]
            rng.shuffle(words)  # shuffle index of real password
            entries.append(
                SweetwordList(
                    user_id=str(idx),
                    sweetwords=words,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Attack GPT honeywords with hashmob attacker")
